...
import ParentRepository from './ParentRepository';
 ...
 export default class User {
  ...
  populateHydrationData(hydrationData) {
    if (hydrationData) {
      this.ouncesRecord = new HydrationRepository(hydrationData, this, 'hydration', this.date);
    } else {
      return [];
    }
  }
  populateActivityData(activityData) {
    if (activityData) {
      this.activityRecord = new ActivityRepository(activityData, this, 'activities', this.date);
    } else {
      return [];
    }
  }
  populateFriendsData(userData, activityData, hydrationData, sleepData, date) {
    if (userData) {
      let matchedFriends = userData.filter(user => this.friends.includes(user.id));
      this.friendObjects = matchedFriends.reduce((friendObjects, friend) => {
        friendObjects.push(new User(friend, userData, activityData, hydrationData, sleepData, date))
  ...
  populateUserData(userData, activityData, hydrationData, sleepData, date) {
    ...
    this.populateFriendsData(userData, activityData, hydrationData, sleepData, date)
  ...
  findTrendingStairsDays() {
    ...
    this.activityRecord.dataObjectArray.forEach((activityObject, i, record) => {
      if (record[i + 1] && record[i].flightsOfStairs > record[i + 1].flightsOfStairs) {
